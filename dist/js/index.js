...
//定义头部的底层内容自动切换
    function hWheel(){
        ...
        var newP = $(`<p>${aP.eq(0).html()}</p>`);
        oArticle.append(newP);
        ...
        function wp_content(){ //wheel planting
            oArticle.animate({left:-1000*iNow},1000,function(){
                if(iNow == 3){
                    iNow = 0;
                    oArticle.css("left",0);
                }
        ...
        $.ajax({
            ...
            //拿到图片数据，循环遍历渲染到页面中，此时页面中的图片都是绝对定位，重叠在一起，最后一张加入结构中的图片层级最高，显示在最上面
                for(var i = 0; i < arr.length;i++){
                    var img=$(`<img src="${arr[i].img}" alt="">`)
                    img.prependTo($(".img_box"))
                }
            //初始化banner导航样式
                $(".b_nav li").eq(0).addClass("active");
                
            //定义图片及图片导航移动事件函数
                 function play(num){
                    $(".b_nav li").eq(num).addClass("active").siblings().removeClass("active");
                    $(".img_box img").eq(3-num).fadeIn(1000).siblings().fadeOut(1000);

                }  
        
            //给banner导航添加移入事件，并设置鼠标移入某一个子导航时会显示对应的图片，且同时清除定时器
                 $(".b_nav li").on("mouseenter",function(){
            ...
            //给banner导航添加移出开始定时器事件
                $(".b_nav li").on("mouseleave",function(){
                    var index = $(this).index();
                    timer = setInterval(function(){
                        index++
                        play(index);
                ...
                //给图片添加移入停止定时器
                $(".img_box img").on("mouseenter",function(){
                    clearInterval(timer)
                })
        ...
        $.ajax({
            ...
            <div class="p_line">
                ...
                for(var i = 0; i<arr.length;i++){
                    ...
                    <div class="split"></div>
                    <div class="row_content"></div>
                     <a href="${arr[i].href}" class="view_all_btn active">
                         ${arr[i].select}
